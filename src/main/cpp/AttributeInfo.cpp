...
ConstantValue ConstantValue::read(FILE *fp, AttributeInfo attribute_info) {
...
CodeAttribute CodeAttribute::read(FILE *fp, AttributeInfo attribute_info) {
    unsigned int i, j, k;
...
InnerClass InnerClass::read(FILE *fp, AttributeInfo attribute_info) {
    attribute_info.name_index = TwoByte.byteCatch(fp);
    attribute_info.length = FourByte.byteCatch(fp);
...
Synthetic Synthetic::read(FILE *fp, AttributeInfo attribute_info) {
    /* A class member that does not appear in the source code must be marked using a Synthetic attribute */
}

Exception Exception::read(FILE *fp, AttributeInfo attribute_info) {
    int i;
...
AttributeInfo AttributeInfo::get_attribute_info(FILE * fp, AttributeInfo attribute_info) {
    attribute_info.name_index = TwoByte.byteCatch(fp);
    attribute_info.length = FourByte.byteCatch(fp);
